...
class GraspController:
    ...
    def get_3d_position(self, bbox, depth_image):
        ...
        return self._align_point_2d_to_3d(cx, cy, z,
                                          self._detector.depth_intrinsic)
    ...
    def execute_grasp_sequence(self, x, y, z):
        ...
        try:
            ret = self.arm.rm_set_manual_tool_frame(frame)
    ...
    def run(self):
        ...
        try:
            ...
            arm.rm_movej_p([-0.254, -0.00001, 0.091, 3.113, 0.0, -1.572],
                           self.ARM_SPEED, 0, 0, 1)
            # 夹爪初始化准备
            self.arm.rm_set_modbus_mode(1, 115200, 1)
            write_params = rm_peripheral_read_write_params_t(1, 1, 2)
            self.arm.rm_write_single_coil(write_params, 1)
